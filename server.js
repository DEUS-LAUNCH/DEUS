 ...
 * WALLET_SECRET=your_base58_encoded_wallet_private_key
 * SOLANA_RPC_URL=https://your-rpc-provider.com
 * ADMIN_WALLET=your_admin_wallet_public_key (optional)
...
app.post('/create', upload.single('image'), async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            imageUrl = `https://gateway.pinata.cloud/ipfs/${imageUploadResult.cid}`;
            fs.unlinkSync(req.file.path);
        }
        ...
        const uri = `https://gateway.pinata.cloud/ipfs/${jsonUploadResult.cid}`;
...
// Serve the SPA for token pages
app.get("/token/:mint", (req, res) => {
  res.sendFile(path.join(__dirname, "public", "index.html"));
});

// Optional: serve SPA for any other non-API route
app.get(/^\/(?!api\/).*/, (req, res) => {
  res.sendFile(path.join(__dirname, "public", "index.html"));
});
...
server.listen(PORT, () => {
    console.log(`JUNKNET server running on http://localhost:${PORT}`);
    // --- KICK OFF ALL BACKGROUND PROCESSES ---
});